...
const _numberWords = ['один', 'два', 'три', 'четыре', 'пять', 'шесть', 'семь', 'восемь', 'девять', 'десять'];
...
void main() {
  group('WordPracticeScreen', () {
    ...
    testWidgets('restricting to one category only ever prompts words from it', (tester) async {
      ...
      // Cycle through every prompt in the (10-word) Numbers pool and check
      // each one is actually a number word.
      for (var i = 0; i < _numberWords.length; i++) {
        final shown = _numberWords.where((w) => find.text(w).evaluate().isNotEmpty);
